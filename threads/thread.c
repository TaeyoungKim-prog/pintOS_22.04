...
thread_yield (void) {
	...
	old_level = intr_disable ();
	if (curr != idle_thread)
		list_insert_ordered (&ready_list, &curr->elem, priority_comparison, 0);
...
bool 
priority_comparison(const struct list_elem *l, const struct list_elem *s, void *aux UNUSED)
{
	int first = list_entry (l, struct thread, elem)->priority;
	int second = list_entry (s, struct thread, elem)->priority;
